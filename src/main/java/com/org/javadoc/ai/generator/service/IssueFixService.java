...
public class IssueFixService {
    ...
    private final PullRequestMetricsRepository pullRequestMetricsRepository; // No need for @Autowired

    public IssueFixService(JavaCodeParser javaCodeParser, GitHubUtility gitHubUtility,
                           PullRequestMetricsRepository pullRequestMetricsRepository) { // Constructor injection
    ...
    public CompletableFuture<String> startFix(String operationId, List<ClassDescription> classDescriptions) {
        ...
        try {
            ...
            pullRequest = gitHubUtility.createPullRequest(sonarIssues.entrySet().stream().map(Map.Entry::getKey).toList(), "Automated fixing issues");
